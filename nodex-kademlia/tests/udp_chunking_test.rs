...
fn test_chunk_assembly_and_checksum_verification() {
    ...
    let src_addr: std::net::SocketAddr = "127.0.0.1:50000".parse().unwrap();
    let mut completed = None;
    for chunk in chunks {
        if let Some(res) = assembler.add_chunk(src_addr, chunk) {
            completed = Some(res);
        }
    }
...
fn test_corrupted_chunk_rejected() {
    ...
    let src_addr: std::net::SocketAddr = "127.0.0.1:50001".parse().unwrap();
    let mut assembler = ChunkAssembler::new(Duration::from_secs(5));
    assert!(assembler.add_chunk(src_addr, chunks[0].clone()).is_none());
}
