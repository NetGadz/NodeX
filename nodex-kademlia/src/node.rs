...
pub const ID_SIZE: usize = core_ffi::ID_SIZE;
...
impl NodeId {
    ...
    pub fn from_key(key: &[u8]) -> Self {
        Self(core_ffi::ffi_hash_node_id(key))
    }
    ...
    pub fn shared_prefix_len(&self, other: &NodeId) -> usize {
        core_ffi::ffi_shared_prefix_bits(&self.0, &other.0) as usize
    }
...
impl KBucket {
    ...
    pub fn force_update(&mut self, contact: Contact) -> UpdateResult {
        if let Some(pos) = self.contacts.iter().position(|c| c.id == contact.id) {
...
impl RoutingTable {
    ...
    pub fn update(&mut self, contact: Contact) -> UpdateResult {
        ...
        if contact.addr.port() == 0 || contact.addr.ip().is_unspecified() || contact.addr.ip().is_multicast() {
            return UpdateResult::SelfIgnored;
        }
    ...
    pub fn force_update(&mut self, contact: Contact) -> UpdateResult {
        ...
        let idx = self.bucket_index(&contact.id);
        self.buckets[idx].force_update(contact)
    }
