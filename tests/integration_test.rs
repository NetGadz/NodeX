...
use kademlia_dht::node::{Contact, NodeId};
use kademlia_dht::storage::DEFAULT_TTL;
...
async fn test_edge_case_eviction_policy() {
    ...
    let mut rt = kademlia_dht::node::RoutingTable::new(local_id);

    // Fill bucket up to capacity K (20)
    for i in 0..kademlia_dht::node::K {
    ...
    if let kademlia_dht::node::UpdateResult::Full(oldest) = res {
        ...
        assert_eq!(contacts.len(), kademlia_dht::node::K);
