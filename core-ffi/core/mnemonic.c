...
// Standard BIP-39 Compact wordlist (2048 words)
// For space efficiency and zero dependencies, we provide the curated BIP-39 words table
static const char *BIP39_WORDS[MNEMONIC_WORD_COUNT] = {
    ...
    "pin", "pinch", "pipe", "pistol", "pitch", "pizza", "place", "planet", "plastic", "plate",
...
// Simple SHA-256 for mnemonic checksum and key derivation
// 32-bit big-endian rotation helpers
...
static void sha256_block(const uint8_t *chunk, uint32_t *h) {
    uint32_t w[64];
    for (int i = 0; i < 16; i++) {
    ...
    for (int i = 0; i < 64; i++) {
        uint32_t t1 = h_val + EP1(e) + CH(e, f, g) + K256[i] + w[i];
...
static void c_sha256(const uint8_t *data, size_t len, uint8_t *out32) {
    uint32_t h[8] = {
    ...
    size_t pos = 0;
    while (pos + 64 <= len) {
        sha256_block(data + pos, h);
        pos += 64;
    }

    size_t rem = len - pos;
    memcpy(buffer, data + pos, rem);
    buffer[rem++] = 0x80;
    if (rem > 56) {
        memset(buffer + rem, 0, 64 - rem);
        sha256_block(buffer, h);
        rem = 0;
    }
    memset(buffer + rem, 0, 56 - rem);
    uint64_t bit_len = (uint64_t)len * 8;
    for (int i = 0; i < 8; i++) {
        buffer[63 - i] = (uint8_t)(bit_len >> (i * 8));
    }
    sha256_block(buffer, h);
...
int c_mnemonic_generate_12(const uint8_t *entropy_16, char *out_words_buf, size_t buf_len) {
    if (!entropy_16 || !out_words_buf || buf_len < 128) return -1;

    // 128 bits entropy + 4 bits checksum = 132 bits = 12 words * 11 bits
    uint8_t hash[32];
    c_sha256(entropy_16, 16, hash);
    uint8_t checksum = hash[0] >> 4; // top 4 bits

    uint8_t bits[17];
    memcpy(bits, entropy_16, 16);
    bits[16] = checksum << 4;

    uint16_t word_indices[12];
    // Convert 132 bits into twelve 11-bit numbers
    // 0..15 bytes = 120 bits + 12 bits from byte 15,16
    for (int i = 0; i < 12; i++) {
        int bit_idx = i * 11;
        ...
        val = (val >> (24 - 11 - bit_offset)) & 0x7FF;
        word_indices[i] = (uint16_t)val;
    }

    out_words_buf[0] = '\0';
    size_t written = 0;
    for (int i = 0; i < 12; i++) {
        const char *w = BIP39_WORDS[word_indices[i] % MNEMONIC_WORD_COUNT];
        size_t wlen = strlen(w);
        if (written + wlen + 2 >= buf_len) return -2;
        if (i > 0) {
            out_words_buf[written++] = ' ';
        }
        memcpy(out_words_buf + written, w, wlen);
        written += wlen;
        out_words_buf[written] = '\0';
    }
...
static int find_word_index(const char *word) {
    for (int i = 0; i < MNEMONIC_WORD_COUNT; i++) {
...
int c_mnemonic_to_seed(const char *mnemonic_str, uint8_t *out_seed_32) {
    if (!mnemonic_str || !out_seed_32) return -1;

    char copy[256];
    strncpy(copy, mnemonic_str, sizeof(copy) - 1);
    ...
    if (count != 12) return -2; // Must be exactly 12 words

    uint16_t indices[12];
    for (int i = 0; i < 12; i++) {
        int idx = find_word_index(words[i]);
        if (idx < 0) return -3; // Invalid word
        indices[i] = (uint16_t)idx;
    }

    // Deterministically hash the mnemonic words with salt "mnemonic_nodex_v1"
    uint8_t salt_prepended[320];
    const char *salt = "NODEX_MASTER_SEED_BIP39_";
    size_t salt_len = strlen(salt);
    memcpy(salt_prepended, salt, salt_len);
    size_t mlen = strlen(mnemonic_str);
    if (salt_len + mlen >= sizeof(salt_prepended)) mlen = sizeof(salt_prepended) - salt_len - 1;
    memcpy(salt_prepended + salt_len, mnemonic_str, mlen);

    c_sha256(salt_prepended, salt_len + mlen, out_seed_32);
    return 0;
}

int c_mnemonic_validate(const char *mnemonic_str) {
    uint8_t dummy[32];
    return c_mnemonic_to_seed(mnemonic_str, dummy) == 0 ? 1 : 0;
}
