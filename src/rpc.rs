...
use crate::node::{Contact, NodeId, RoutingTable, UpdateResult, ID_SIZE};

pub const MAX_VALUE_SIZE: usize = 4096;
pub const MAX_CONTACTS: usize = 20;

// Message Types matching serialize.h
...
impl RpcMessage {
    ...
    pub fn encode(&self) -> Result<Vec<u8>, String> {
        ...
        let written = unsafe { encode_message(&c_msg, buf.as_mut_ptr(), buf.len()) };
        if written < 0 {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self, String> {
        let mut c_msg: CMessage = unsafe { std::mem::zeroed() };
        let res = unsafe { decode_message(bytes.as_ptr(), bytes.len(), &mut c_msg) };
        if res < 0 {
