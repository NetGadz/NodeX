...
impl KademliaNode {
    ...
    pub async fn start_with_config(config: NodeConfig) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Attempt to load node state if state_file exists
        let (node_id, restored_contacts) = if std::path::Path::new(&config.state_file).exists() {
            match StateManager::load_state(&config.state_file) {
                Ok((id, contacts)) => {
                    println!("[STATE] Restored persistent Node ID {} and {} saved contacts from {}", id, contacts.len(), config.state_file);
                    (id, contacts)
                }
                Err(e) => {
                    println!("[STATE] Failed to load state from {}: {}. Generating new random Node ID.", config.state_file, e);
