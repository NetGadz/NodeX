...
int encode_message(const Message* msg, uint8_t* out_buf, size_t buf_len) {
    ...
    switch (msg->type) {
        ...
        case MSG_STORE: {
            ...
            if (buf_len < pos + ID_SIZE + 2 + msg->payload.store.value_len) return -1;
            memcpy(&out_buf[pos], msg->payload.store.key, ID_SIZE);
            pos += ID_SIZE;
            write_u16_be(&out_buf[pos], msg->payload.store.value_len);
            pos += 2;
        ...
        case MSG_FIND_VALUE_RESP: {
            ...
            if (msg->payload.find_value_resp.has_value) {
                uint16_t vlen = msg->payload.find_value_resp.value_len;
                if (vlen > MAX_VALUE_SIZE) return -1;
                if (buf_len < pos + 2 + vlen) return -1;
                write_u16_be(&out_buf[pos], vlen);
                pos += 2;
                if (vlen > 0) {
...
int decode_message(const uint8_t* buf, size_t len, Message* out_msg) {
    ...
    switch (out_msg->type) {
        ...
        case MSG_STORE: {
            if (len < pos + ID_SIZE + 2) return -4;
            memcpy(out_msg->payload.store.key, &buf[pos], ID_SIZE);
            pos += ID_SIZE;
            uint16_t vlen = read_u16_be(&buf[pos]);
            pos += 2;
